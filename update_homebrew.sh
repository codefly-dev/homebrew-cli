...
# Update the version using GNU sed
sed -i "s/version \".*\"/version \"$VERSION\"/" "$FORMULA"

# Update the URLs using GNU sed
...
for arch in "${!urls[@]}"; do
    ...
    # Update the formula file with the new sha256 for each architecture using GNU sed
    echo "Updating formula ${FORMULA} for ${arch}..."
    sed -i "s|sha256 \".*${arch}\"|sha256 \"${sha256}\" # ${arch}|" "$FORMULA"
